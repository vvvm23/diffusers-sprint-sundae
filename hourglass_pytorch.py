...
class NaiveDownsample(nn.Module):
    ...
    def forward(self, x):
        if self.pixel:
            ...
            return reduce(x, 'b (h sh) (w sw) d -> b (h w) d', 'mean', sh = self.shorten_factor, sw = self.shorten_factor)
...
class NaiveUpsample(nn.Module):
    ...
    def forward(self, x):
        if self.pixel:
            x = rearrange(x, 'b (h w) d -> b h w d', h = int(sqrt(x.shape[1])))
            x = repeat(x, 'b h w d -> b (h sh) (w sw) d', sh=self.shorten_factor, sw=self.shorten_factor)
...
class LinearDownsample(nn.Module):
    ...
    def forward(self, x):
        if self.pixel:
            x = rearrange(x, 'b (h w) d -> b h w d', h = int(sqrt(x.shape[1])))
            x = rearrange(x, 'b (h sh) (w sw) d -> b h w (d sh sw)', sh = self.shorten_factor, sw = self.shorten_factor)
            x = self.proj(x)
            return rearrange(x, 'b h w d -> b (h w) d')
...
class LinearUpsample(nn.Module):
    ...
    def forward(self, x):
        if self.pixel:
            x = rearrange(x, 'b (h w) d -> b h w d', h = int(sqrt(x.shape[1])))
            x = self.proj(x)
            x = rearrange(x, 'b h w (d sh sw) -> b (h sh) (w sw) d', sh = self.shorten_factor, sw = self.shorten_factor)
            return rearrange(x, 'b h w d -> b (h w) d')
        x = self.proj(x)
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert attn_type in ['vanilla', 'performer'], f"attention type '{attn_type}' not recognized!"
        self.layers = nn.ModuleList([])

        AttnModule = Attention if attn_type == 'vanilla' else PerformerAttention
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNormResidual(dim, AttnModule(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout, causal = causal)),
        ...
        self.rotary_pos_emb = RotaryEmbedding(rotary_emb_dim, freqs_for='pixel', max_freq=int(sqrt(max_seq_len))) if rotary_pos_emb else None
    ...
    def forward(self, x, context = None, mask = None):
        ...
        if exists(self.rotary_pos_emb):
            ...
            freqs_h = self.rotary_pos_emb(torch.linspace(-1, 1, steps = w).to(x.device), cache_key=w)
            freqs_w = self.rotary_pos_emb(torch.linspace(-1, 1, steps = w).to(x.device), cache_key=w)
            rotary_pos_emb = broadcat((freqs_h[:, None, :], freqs_w[None, :, :]), dim = -1)
...
class HourglassTransformerLM(nn.Module):
    ...
    def sample(self,
        ...
        if exists(start_latent):
            ...
            batched_text = torch.randint(self.num_tokens, (nb_samples, self.max_seq_len)).to(device)
        ...
        for n, (temperature, sample_proportion) in enumerate(zip(temperatures, proportions)):
            old_sample_mask = sample_mask.clone()
            logits = self.forward(batched_text[~sample_mask], c=None if c == None else c[~sample_mask])
            sample = Categorical(logits=logits/temperature).sample()
            
            mask = (torch.rand(sample.shape) > sample_proportion).to(batched_text.device)
            ...
            pb.set_description_str(f"T: {temperature:.3f}, p: {sample_proportion:.3f}, [{sample_mask.long().sum().item()} / {batched_text.shape[0]}]")
            pb.update(1)
...
if __name__ == '__main__':
    ...
    print(net.sample(10, 16, 1.0, 0.5, device, end_temperature=0.7, end_sample_proportion=1.0, status=True).shape)
